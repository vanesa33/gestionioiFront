...
function TasksFromPage() {
  ...
  // ✅ Un solo estado para “la orden que acabo de crear/usar”
  ...
  useEffect(() => {
    ...
    if (modoEdicion && location.state?.ingreso?.imagenurl) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (selectedFile) {
        ...
        setImageurl(imagenFinal);
        setImageUrlPreviewUrl(imagenFinal);
      }

      // 2️⃣ Crear ingreso con imagen o null (✅ SOLO UNA VEZ)
      ...
      // ✅ Guardamos la orden creada para imprimir/editar
      setUltimoIngreso(res.data);

      const { numorden: orden } = res.data;
            ...
            <div className="w-full md:w-auto flex justify-center md:justify-start">
              <div className="flex flex-col gap-4 mt-6 md:mt-0">
                ...
                <input
                  ...
                  type="file"
                  accept="image/*" // ✅ estaba mal escrito
                ...
                {/* ✅ Si querés ver el estado (debug), dejalo así, dentro del return */}
                {/* <div className="text-gray-700">formBloqueado: {formBloqueado ? "true" : "false"}</div> */}
              </div>
            </div>
          </div>
        </div>
